...
| 화면 | 아이보리 바탕에 배너·탭·버튼만 한 단계씩 진하게. 노선도는 종이 노선도처럼 한 톤 밝게 |
...
역의 **실제 위경도**로 그립니다(`subwayStationMaster`). 그래서 2호선이 순환선 모양이 되고
노선이 교차하는 위치도 실제와 같습니다. 서울교통공사가 배포하는 공식 노선도는 사람이 직선과
45도로 정리한 도안이고 그 좌표는 공개되지 않아 재현하지 않습니다.

- **어떤 역끼리 잇는지는 좌표에서 계산합니다.** 응답에는 인접 정보가 없습니다. 역번호 순서로
  이으면 지선이 본선 사이에 끼어 없는 선로가 생깁니다(신정네거리→문래, 하남시청→둔촌동).
  지선(`-N`) 정보를 먼저 확정하고, 나머지는 가까운 쌍부터 이어 붙입니다
  ([`LineTopology`](app/src/main/java/com/example/gc_last/ui/LineTopology.kt)).
- **환승역**: 같은 이름의 역이 여러 노선에 있으면 환승역으로 보고 흰 원으로 표시합니다(120곳).
- **좌표 없는 역**: 799개 중 31개. 같은 이름의 역이 있으면 그 좌표를 빌려 쓰고
  (수인분당선 오이도·한대앞 등), 없으면 그리지 않습니다(GTX-A 동탄 1곳).
- **역 이름**은 확대해야 나오고, 이미 글자가 놓인 자리와 겹치면 건너뜁니다.
- **확대**는 처음 상태에서 약 11배까지. 도심은 역 간격이 200m 도 안 되는 곳이 있습니다.
- **배경에 시·도 경계**를 옅게 깝니다(통계청 2018 경계, 공개 자료). 노선만 떠 있으면
  어디가 육지인지 알 수 없기 때문입니다. 서울 자치구 경계까지 넣으면 도심에서 선이 겹쳐
  노선을 읽기 어려워 시·도 경계만 씁니다.
